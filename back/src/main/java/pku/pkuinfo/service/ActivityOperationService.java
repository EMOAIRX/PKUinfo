...
public class ActivityOperationService {
     ...
     * 开启Redis缓存
     */
    @Cacheable(value = "activity_week", key = "#startDate+#endDate")
    ...
    public String talking(String text, String appendix) {
        String ques = "现在你正在扮演一个活动信息检索工具，用户想要检索一个信息，用户的输入是：[[" + text + "]]在数据库中的内容有其中一部分和用户的输入相似，内容是：[[" + appendix
                + "]]现在，你需要回答对用户的输入作出反应，你的回答是[请注意简短回答，尽可能简短，但对话应当风趣幽默自然，不要简单陈列活动]：";
    ...
    public List<ActivityInfo> search_tag(String text, Date startDate, Date endDate) {
        ...
        // sort by value from large to small
        // Result.sort((a, b) -> Float.compare(value.get(b.getId()),
        // value.get(a.getId())));
